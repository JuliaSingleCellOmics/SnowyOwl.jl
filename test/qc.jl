@testset "qc" begin
    ...
    SnowyOwl.Preprocess.quality_control_metrics!(data, obs, var, use_log1p=true)
    @test obs[!, names(obs_metrics)] == obs_metrics
    @test var[!, names(var_metrics)] == var_metrics

    prof1 = SnowyOwl.Preprocess.quality_control_metrics(prof, use_log1p=true)
    @test prof1.obs == prof.obs
    @test prof1.RNA.var == prof.RNA.var

    SnowyOwl.Preprocess.quality_control_metrics!(prof1, use_log1p=true)
    @test prof1.obs == prof.obs
    @test prof1.RNA.var == prof.RNA.var
end
